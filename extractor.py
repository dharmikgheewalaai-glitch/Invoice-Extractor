...
def parse_invoice(pdf, text, filename):
    ...
    df["Gross Amount"] = df.apply(
        lambda x: safe_float(x["Gross Amount"])
        if safe_float(x["Gross Amount"]) != 0
    ...
    df["Discount Amount"] = df.apply(
        lambda x: safe_float(x["Discount Amount"])
        if safe_float(x["Discount Amount"]) != 0
        else safe_float(x["Gross Amount"]) * safe_float(x["Discount%"]) / 100,
        axis=1
    )

    df["Discount%"] = df.apply(
        lambda x: safe_float(x["Discount%"])
        if safe_float(x["Discount%"]) != 0
        else (safe_float(x["Discount Amount"]) / safe_float(x["Gross Amount"]) * 100
              if safe_float(x["Gross Amount"]) != 0 else 0),
        axis=1
    )

    # IGST
    df["IGST Amount"] = df.apply(
        lambda x: safe_float(x["IGST Amount"])
        if safe_float(x["IGST Amount"]) != 0
        else (safe_float(x["Gross Amount"]) - safe_float(x["Discount Amount"])) * safe_float(x["IGST%"]) / 100,
        axis=1
    )
    df["IGST%"] = df.apply(
        lambda x: safe_float(x["IGST%"])
        if safe_float(x["IGST%"]) != 0
        else (safe_float(x["IGST Amount"]) / max((safe_float(x["Gross Amount"]) - safe_float(x["Discount Amount"])), 1) * 100),
        axis=1
    )

    # CGST
    df["CGST Amount"] = df.apply(
        lambda x: safe_float(x["CGST Amount"])
        if safe_float(x["CGST Amount"]) != 0
        else (safe_float(x["Gross Amount"]) - safe_float(x["Discount Amount"])) * safe_float(x["CGST%"]) / 100,
        axis=1
    )
    df["CGST%"] = df.apply(
        lambda x: safe_float(x["CGST%"])
        if safe_float(x["CGST%"]) != 0
        else (safe_float(x["CGST Amount"]) / max((safe_float(x["Gross Amount"]) - safe_float(x["Discount Amount"])), 1) * 100),
        axis=1
    )

    # SGST
    df["SGST Amount"] = df.apply(
        lambda x: safe_float(x["SGST Amount"])
        if safe_float(x["SGST Amount"]) != 0
        else (safe_float(x["Gross Amount"]) - safe_float(x["Discount Amount"])) * safe_float(x["SGST%"]) / 100,
        axis=1
    )
    df["SGST%"] = df.apply(
        lambda x: safe_float(x["SGST%"])
        if safe_float(x["SGST%"]) != 0
        else (safe_float(x["SGST Amount"]) / max((safe_float(x["Gross Amount"]) - safe_float(x["Discount Amount"])), 1) * 100),
        axis=1
    )

    # Net Amount
    df["Net Amount"] = df.apply(
        lambda x: safe_float(x["Net Amount"])
        if safe_float(x["Net Amount"]) != 0
